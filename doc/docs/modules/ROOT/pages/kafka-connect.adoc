...
[abstract]
--
This chapter describes Kafka Connect Neo4j Connector in the Neo4j Streams Library.
--
endif::env-docs[]
...
The Neo4j Streams project provides a Kafka Connect Neo4j Connector that can be installed into the Confluent Platform enabling:

- Ingest data from Kafka topics directly into Neo4j via templated Cypher queries;
- Stream Neo4j queries.

[#kafka_connect_neo4j_connector_install]
== Plugin installation
...
In this chapter we'll discuss how the Source instance works
...
[NOTE]
Timestamps aren’t necessarily unique. As a result, this mode can’t guarantee that all updated data will be delivered.
For example, if two rows share the same timestamp, and only one has been pushed to the kafka topic before a failure,
the second update will be missed when the system recovers.
...
[source,json]
----
include::ROOT:partial$docker-data/contrib.sink.string-json.neo4j.json[]
----
...
curl -X POST http://localhost:8083/connectors \
  -H 'Content-Type:application/json' \
  -H 'Accept:application/json' \
  -d @contrib.sink.string-json.neo4j.json
----
...
in order to support this feature; in order to support this feature with Kafka Connect Neo4j Connector, creating the Sink
...
In Kafka Connect Neo4j Connector, in the creation phase of the Sink instance, in addition to the properties
...
Kafka Connect Neo4j Connector supports also the secured Neo4j URI schemes.
